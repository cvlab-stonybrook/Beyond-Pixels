...
#         self.dropout_prantik = nn.Dropout(0.20)
        self.prantik_avgpool1 = nn.AvgPool2d(3, stride=1, padding=1)
        self.prantik_avgpool2 = nn.AvgPool2d(5, stride=1, padding=2)
        self.prantik_avgpool3 = nn.AvgPool2d(9, stride=1, padding=4)
        self.prantik_avgpool4 = nn.AvgPool2d(17, stride=1, padding=8)
#####################################################################################################
#         self.convolution_prantik = nn.Sequential( ##########################################################################68.29
#                                                 #########################66.88(only reduction)
...
#                                                 )
        ...
        self.convolution_prantik = nn.Sequential( ##########################################################################68.29
                                                 nn.Conv2d(1280,1024,  kernel_size=1, stride=1, bias=False),
                                                 nn.BatchNorm2d(1024),
                                                 nn.ReLU(True),
...
#########################################################################################################68.61
        ...
        self.classifier_prantik_2 = nn.Linear(32, cfg['nclass'])
        self.dropout_prantik = nn.Dropout(0.20)
...
#         self.classifier_prantik_2 = nn.Linear(32, 19)
    ...
    def forward(self, x, need_fp=False, classify=False, nlabel=0, val_feat=False):
        ...
        #print("model3", c1.shape, c4.shape)#[2, 256, 81, 81] [2, 2048, 21, 21])
...
################################################################68.61
                ...
                c1_avgpool3 = self.prantik_avgpool3(c1)
                c1_avgpool4 = self.prantik_avgpool4(c1)
                final = torch.cat((c1,c1_avgpool1,c1_avgpool2,c1_avgpool3,c1_avgpool4),dim=1)
                c1_lb = self.convolution_prantik(torch.cat( (final,nn.Dropout2d(0.5)(final))))#[4, 64, 201, 201]
                c1_lb = c1_lb.view(c1_lb.shape[0],c1_lb.shape[1],-1)
                c1_lb = c1_lb.permute(0,2,1)#torch.Size([4, 40401, 64])
                classifier_feats = F.leaky_relu(self.classifier_prantik_1(c1_lb))
                ...
                classifier_feat, classifier_feat_fp = classifier_feats.chunk(2)#[2, 6561, 21],[2, 6561, 21]
#################################################################
        ...
        if classify:
            c1_avgpool = self.prantik_avgpool(c1)
            c1_avgpool = self.convolution_prantik(c1_avgpool)
            c1_avgpool = c1_avgpool.view(c1_avgpool.shape[0],c1_avgpool.shape[1],-1)
            c1_avgpool = c1_avgpool.permute(0,2,1)#torch.Size([2, 40401, 256])
            classifier_feats = F.leaky_relu(self.classifier_prantik_1(c1_avgpool))
            classifier_feats = self.dropout_prantik(classifier_feats)
